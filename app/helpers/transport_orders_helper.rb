module TransportOrdersHelper
	def generate_transport_order (tr_id, bid_id)
		@contract = Contract.where(:transporter_id => tr_id, :bid_id => bid_id, :signed => true)&.first
		if (! @contract.present?)
			return false
		end
		@transport_requisition = TransportRequisition.find(tr_id)
		@uom = UnitOfMeasure.where(:code => 'MT').first
		@transport_requisition.transport_requisition_items.where(:has_transport_order => false)
		.find_each do |tr_item|
			...
			@bid_quotations.each do |bid_quotation|
				@transport_order = TransportOrder.where({:transporter_id => bid_quotation.transporter_id, :bid_id => bid_id, :operation_id => @transport_requisition.operation_id}).where.not({status: [:closed, :canceled, :archived] }).first
				if (@transport_order.present?)
					...
					@new_to = TransportOrder.new(order_no: SecureRandom.uuid, contract_id: @contract.id, transporter_id: bid_quotation.transporter_id, bid_id: bid_id, operation_id: @transport_requisition.operation_id, region_id: @woreda.parent.parent_node_id, order_date: Time.current, created_date: Time.current, start_date: 3.days.from_now, end_date: 13.days.from_now, printed_copies: 0, status: 0, created_by: current_user.id, transport_requisition_id: @transport_requisition.id)
					@new_to.save
	...
	def to_for_exception (tr_id, woreda_id, transporter_id, tariff)
		@transport_requisition = TransportRequisition.find(tr_id)
		...
		@transport_requisition.transport_requisition_items.joins(fdp: :location).where(:has_transport_order => false, :'locations.id' => woreda_id)
			.find_each do |tr_item|
				...
				if (@transport_order.present?)
					@new_to_detail = TransportOrderItem.new(transport_order_id: @transport_order.id, fdp_id: tr_item.fdp_id, commodity_id: tr_item.commodity_id, quantity: tr_item.quantity, unit_of_measure_id: @uom.id, tariff: tariff, requisition_no: @requisition.requisition_no, created_by: current_user.id)
					@new_to_detail.save
					tr_item.has_transport_order = true
					tr_item.save
				else
					@new_to = TransportOrder.new(order_no: SecureRandom.uuid, transporter_id: transporter_id, operation_id: @transport_requisition.operation_id, region_id: @woreda.parent.parent_node_id, order_date: Time.current, created_date: Time.current, start_date: 3.days.from_now, end_date: 13.days.from_now, printed_copies: 0, status: 0, created_by: current_user.id, transport_requisition_id: @transport_requisition.id)
					@new_to.save
					@new_to.order_no = "TRN-ORD-" + @new_to.id.to_s	
					@new_to.save				
					@new_to_detail = TransportOrderItem.new(transport_order_id: @new_to.id, fdp_id: tr_item.fdp_id, commodity_id: tr_item.commodity_id, quantity: tr_item.quantity, unit_of_measure_id: @uom.id, tariff: tariff, requisition_no: @requisition.requisition_no, created_by: current_user.id)
					@new_to_detail.save
					tr_item.has_transport_order = true
					tr_item.save
				end
