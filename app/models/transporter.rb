...
class Transporter < ApplicationRecord
    ...
    def self.fdp_allocations (transporter_id, operation_id, requisition_nos)
          ...
          @dispatch_summary = []
          Requisition.joins(:commodity, :operation, requisition_items: [:fdp]).where({:'requisitions.operation_id' => operation_id} ).where('requisitions.requisition_no IN (?)',  requisition_nos).where('requisition_items.amount > 0').uniq{|t| t.requisition_no }.each do |allocation|          
