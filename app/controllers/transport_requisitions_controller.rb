class TransportRequisitionsController < ApplicationController  
  ...
  def show   
    ...
    @transport_orders = TransportOrder.includes(:transporter, :bid, transport_order_items: [transport_requisition_item: :transport_requisition]).where(:'transport_requisition_items.transport_requisition_id' => params[:id])
    @transport_requisition = TransportRequisition.includes(transport_requisition_items: [:commodity, fdp: :location, transport_order_items: :transport_order, requisition: [:region, :zone] ]).find(params[:id])
